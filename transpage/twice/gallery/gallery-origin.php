...
<head>
    ...
    $(function(){
        ...
        page = Number(page[0].split('gallery')[1]);
        
        $.getJSON("../lib/json/gallery.json", function(e){
            var total = Number(Math.ceil(e.gallery.length / 30));
            
            $.each(e.gallery, function(i, field){
                //갯수 조절, 전체 갯수와 페이지 수로 비교해서 30개만 출력
                
                if(i >= Math.abs(page - total)*30 && i < Math.abs(page - (total + 1)) * 30){
            ...
            for(i=0; i < parseInt(Number(e.gallery.length) / 30) + 1; i++){
                $('.list-nav ul').append('<li><a href="gallery'+(parseInt(Number(e.gallery.length) / 30) - (i - 1))+'.php" class="list-nav-off">'+i+'</a></li>')
            }
            $('.list-nav ul li').eq(Math.abs(page - total)).children('a').removeClass('list-nav-off').addClass('list-nav-on');
        });
